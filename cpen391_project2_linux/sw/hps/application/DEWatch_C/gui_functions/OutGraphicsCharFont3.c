...
void OutGraphicsCharFont3(int x, int y, int colour, int backgroundcolour, int c, int Erase)
{
	...
	if(((short)(theCharacter) >= (short)(' ')) && ((short)(theCharacter) <= (short)('~'))) {			// if printable character
		...
		theRow = FONT3_YPIXELS * 2;
		theColumn = FONT3_XPIXELS;

		for(row = 0; row < theRow ; row = row + 2)	{
			pixels = Font16x27[theCharacter][row+1] + (((unsigned int)Font16x27[theCharacter][row]) << 8) ;		     								// get the pixels for row 0 of the character to be displayed
			int n = pixels;
			int count = 0;
			while(n)
			{
			...
			for(column = 0; column < theColumn;   )  	{
				if((pixels & BitMask)){														// if valid pixel, then write it
					WriteAPixel(theX+column, theY+row/2, theColour);
					hit++;
				}
				else {
					// if not a valid pixel, do we erase or leave it along (no erase)
					if(Erase == 1)
						WriteAPixel(theX+column, theY+row/2, backgroundcolour) ;
					// else leave it alone
				}
					column ++ ;
				BitMask = BitMask >> 1 ;
